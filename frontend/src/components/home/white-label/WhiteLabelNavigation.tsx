...
type WhiteLabelNavigationProps = {
  currentIndex: number;
  onPrev: () => void;
  onNext: () => void;
...
export const WhiteLabelNavigation = ({
  currentIndex,
  onPrev,
  onNext,
  onGoTo,
}: WhiteLabelNavigationProps) => (
      ...
      {whiteLabelSlides.map((_, index) => (
        <button
          ...
          className={`carousel-dot rounded-full transition-all ${
            currentIndex === index ? "w-6 bg-indigo-600" : "w-2 h-2 bg-gray-300 hover:bg-gray-400"
          }`}
          style={{ height: "8px" }}
